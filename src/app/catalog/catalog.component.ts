...
export class CatalogComponent {
  ...
  //private httpSvc: ProductService = inject(ProductService)
  ...
  getDiscountClasses(product: IProduct) {
    if (product.discount > 0) return ['linethrough'];
    else return [];
    //return {linethrough: product.discount > 0};
  }
  getStyles(product: IProduct)  {
    return product.discount > 0 ? ['green'] : [''];
  }
